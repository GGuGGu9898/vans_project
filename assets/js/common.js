$(document).ready(function () {
    ...
    $('.mnavOpenBtn').on('click', function() {
        ...
        _mGnb.find('.m_Gnb > ul > li > a').on('click', function (e) {
            e.preventDefault();
            $(this).next().stop().slideToggle().parent().toggleClass('on').siblings().removeClass('on').find('> ul').stop().slideUp();
        });
    ...
    _mGnb.find('.btn_close').on('click', function() {
        _mGnb.stop().animate({top:'-100%'},function () {
            $(this).css({visibility : 'hidden'}).find('.m_Gnb > ul > li.on').removeClass('on').find('> ul').hide();
